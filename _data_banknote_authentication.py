...
_curentdatapath = os.path.join(_DataSetPath, "data_banknote_authentication.data")
...
test_dataset = testData[:, :-2].tolist()
test_target = testData[:, -2:].tolist()

dataset = trainData[:, :-2].tolist()
target = trainData[:, -2:].tolist()


myNet5 = net.Dense()
myNet5.AddLayer(4, isInput=True)
myNet5.AddLayer(5, activationFunction=af.Sigmoid)
myNet5.AddLayer(2, activationFunction=af.Sigmoid)

myNet20 = net.Dense()
myNet20.AddLayer(4, isInput=True)
myNet20.AddLayer(20, activationFunction=af.Sigmoid)
myNet20.AddLayer(2, activationFunction=af.Sigmoid)

start_time = time.time()
myNet5.Train(dataset, target, iterationCount= 50, learningRateStart=.6, learningRateEnd=.0, regulationRate=.02)
train5_Time = time.time() - start_time
start_time = time.time()
myNet20.Train(dataset, target, iterationCount= 50, learningRateStart=.6, learningRateEnd=.0, regulationRate=.02)
